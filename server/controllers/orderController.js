  ...
  try {
    ...
    for (const order of orders) {
      ...
      try {
        ...
        console.error(`Failed to decrease stock for product ${data.productId}:`, err);
        failedOrders.push({ order: result, error: err.message });
      }
    }

    if (failedOrders.length > 0) {
      return res.status(207).send({
        message: `Some orders processed, but some failed to update stock`,
        successfulOrders,
        failedOrders
      });
    }

    res.status(200).send({ message: `All orders successfully received`, successfulOrders });
  } catch (error) {
